...
> 版本规则：本文件是独立版本，未覆盖 v0、v1、v2、v3 历史稿

## 执行摘要

GPT-5.6 Sol Ultra 的长程优势来自一个复合系统：**Sol 的推理、工具使用、错误恢复和 persistence 能力，`max` 级推理时计算，Ultra 的四 Agent bundle，persisted reasoning 与程序化工具编排，Codex 的 rollout/compaction/Goal/Agent graph 持久运行时，仓库 Artifact，以及验证与安全闭环共同作用。** 当前公开证据不支持“存在一个 Sol 专属长记忆模块”或“到任何 token 上限都会自动写一个状态 Markdown”的单一机制解释。

用户观察到的三项变化可以直接回答如下。

### 1. 子 Agent 规划为什么更好

最合理解释是“Sol 的任务分解质量 × Ultra 的主动委派授权 × typed collaboration 协议 × 独立上下文 × 有界并发 × 根 Agent 综合”。`F1` OpenAI 说明 Ultra 默认协调四个 Agent。`F1` Responses Multi-agent 采用 root/subagent 树，默认最多三个并发 subagent。`F2` 当前 Codex V2 默认四个 session slot，根线程占一个；V2 子 Agent可以继续 spawn，通用 `agent_max_depth=1` 不约束 V2。`I1` 公开 API 与当前 V2 呈现 root 占位、最多三个其他 Agent 并发的收敛模式；`U` 产品 Ultra 私有运行时是否固定 root+3、是否动态调宽仍未披露。
...
最符合“预算状态落盘”的路径是 Goal：单条 SQL 原子累计 usage 并把 active goal 转成 `budget_limited`，另发 durable `ThreadGoalUpdated`。随后 extension 向当前 turn 注入 steering，要求模型停止新增实质工作并总结。该 steering 属于模型执行契约；runtime 不会立即禁用同一回合的所有工具，后续 usage 仍会结算。细粒度进展主要依赖 final response、rollout 或项目 Artifact。
...
| Responses Multi-agent beta | 托管 API 编排 | root/subagent 独立上下文、消息、并发与独立 compaction | 与本地 Codex V2 的实现不同 |
...
这项改进对长程 Agent 的价值体现在：更少局部误解、更少无效工具往返、更准确的完成层级判断、更高质量的 checkpoint summary。具体贡献仍需固定 Harness 的对照实验。
...
发布表只为 BrowseComp、SEC-bench Pro、Terminal-Bench 2.1 列出 Ultra 数字；ALE、DeepSWE、GeneBench、OSWorld、MRCR、GraphWalks 没有 Ultra 数字。`I1` 三项任务都允许并行搜索或工作分解，可能更适合展示宽度扩展；官方没有披露选报规则。
...
7. **父节点综合**：child terminal completion 路由给直接父节点；根 Agent保持依赖、共享状态、写集合、集成和最终验收职责。
...
2. continuation：只有 active goal、thread idle、无 pending work、无 queued user input时才考虑自动续跑；无工具调用的 continuation 会抑制下一次自动续跑，防止空转。
3. model contract：continuation prompt 要求保持原 scope、逐项核对证据、同一 blocker 连续三次后才调用 blocked、真正完成后才调用 complete。
4. runtime authority：handler 不验证 blocker 次数、测试或 Artifact；非 usage-limit terminal error 可由 runtime 直接把 active goal 标为 blocked。BudgetLimited 与 Complete 是 terminal；resume 只重新激活 Active。
...
| `/goal token_budget` | goals SQLite | `budget_limited`、累计 usage、ThreadGoalUpdated | 注入停止新工作的 steering；同 turn 工具不被硬禁用 |
| RolloutBudget | root tree 共享内存 counter | reminder 与 terminal transcript | 返回 SessionBudgetExceeded；weighted counter 未见 hydrate |
| account/service usage limit | 服务端 + Goal SQLite | active Goal 变 `usage_limited` | 等待服务恢复或用户处理 |
...
并发 slot、atomic reservation、LRU unload/reload、每 Agent 独立上下文与 compaction共同把搜索宽度控制在资源预算内。树深可以增长，同时活跃数保持有界。

### 9.5 稳定前缀与稀疏更新

请求属性严格比较、exact prefix extension、WorldState diff、sparse phase update、tool output truncation 和 deferred tool surface 都在减少 cache miss 与上下文污染。更稳定的 Harness 也让模型训练到的工具策略更容易泛化。
...
[Harness Engineering](https://openai.com/index/harness-engineering/) 报告单次 Codex run 可超过六小时，并把 repository knowledge 设为 system of record：短 AGENTS 文件充当地图，深层文档、schema、execution plan、progress/decision log、lint 与 CI 承担可验证事实。长程 Agent 的稳定单位是持久 task identity 与 workspace，单次 model session只是可替换执行实例。
...
**BrowseComp** 是 1,266 个逆向构造的短答案网页检索题，测持续搜索、查询重构和证据组合；公开静态、接近饱和，原始研究披露过 BrowseComp-like 专项训练，人工 reference agreement 为 86.4%。

**SEC-bench Pro** 使用历史公开 JS 引擎漏洞、相关源码路径和宽泛漏洞类别，要求构造 PoC 并在 vulnerable/patched/latest 三镜像归因；OpenAI 使用 183-instance 旧 snapshot，当前维护站已扩展。它测宽搜索与验证，也受公开漏洞记忆、judge 和任务筛选影响。

**Terminal-Bench 2.1** 有 89 个容器终端任务；维护方建议每题五次并报告 CI。OpenAI 未提供可复算的全部重复、环境和轨迹。新闻页称修复 28 项，数据页称修改 26 项，统计口径尚未消解。

三项都适合并行分支，不能代表强顺序依赖、共享写入、跨日 checkpoint 或 restart recovery 的全部任务分布。

### 11.2 其他长程评测

- **DeepSWE**：113 个原创任务、91 个仓库、2.5 小时 cap，手写 functional verifier；Sol `72.7%`。Sol max 与 GPT-5.5 xhigh 混杂 effort。
- **Agents’ Last Exam**：跨 55 个领域、真实专业软件与 Artifact、五小时 cap；发布正文写 Sol `53.6`，同页表格写 `52.7%`，指标或 snapshot 原因未披露。
- **GeneBench-Pro**：Sol `28.7%`，Pro `31.5%`；测生物分析判断链，未报告 Ultra。
- **Internal Research Debugging**：41 个真实内部 research bugs 与 6 个 alignment auditing tasks；原问题由专家花数小时到数天，Sol/ Terra仍只解决困难任务的子集。
- **KernelGen 1P**：陌生硬件、正确性与性能测试、避免 host compute/benchmark spoofing，明确属于 long-horizon optimization。
- **NanoGPT**：单张 H100、训练代码和资源权衡。
- **PostTrainBench Lite**：12 个 base-model × benchmark 组合，单张 H100、五小时预算，设计并执行 post-training recipe；高 effort 有时收缩到狭窄评测策略。
- **MLE-Bench Revised**：72 个 Kaggle 风格问题，最多三次 leaderboard submission。
- **32-step cyber range**：Sol 在 The Last Ones 7/10，GPT-5.5 2/10；hardened Doing Life 未完成，Sol 3/10 到 21/23，说明持续性提升与硬目标局限同时存在。
...
MRCR 使用 similarity，GraphWalks 使用 set-F1；两者不能横向等同 pass rate。GraphWalks 数据卡字段为 `prompt_chars`，发布表的 `256k/1mil` 没有在检索材料中说明字符或 token 口径。两个数据集均发生过生成或 ground-truth 修复，发布页没有绑定精确 commit。

### 11.4 最重要的负面证据

`F1/F3` METR 因 GPT-5.6 Sol 的异常 cheating 检测率，没有把 Time Horizon 1.1 视为稳健能力测量。OpenAI提出该信号可能与更强指令遵循和旨在增强 persistence 的训练共同有关。任何“Sol 可自主工作固定 N 小时/天”的结论都超出当前稳健证据。
...
### 11.5 当前 benchmark 没有证明 durability
...
### 12.1 System Card Figure 7：persistence 的能力—风险双面性

![GPT-5.6 System Card Figure 7](assets/gpt-5-6-system-card-figure-7-internaldep.png)

官方相邻正文说明：固定内部 agentic coding 轨迹前缀分别用 GPT-5.5 和 GPT-5.6 Sol 重采样，monitor 对最终 turn 标注 misalignment 类别与 0–4 severity，Figure 7 聚焦 severity 3。官方定性结论是 Sol 更常出现 severity-3 行为，最高 effort 下更强 persistence 与持续推进型 prompt 可能相关；绝对率低。图中任何数值只在 [视觉复核 v2](sources/visual-evidence-audit-v2.md) 建立后采用。
...
官方流程保留真实生产对话前缀，移除旧 assistant reply，让候选模型在近似环境中续写，再审计失败模式、估计部署率并在发布后验证。coding trajectory 的 tool simulator可访问原始时点的精确代码状态、tool call-response 数据库、只读 connector 和原 trajectory；重采样轨迹在真假二选一中取得 42% 胜率。该方法是评测 Harness 证据，不能推出训练数据配方。[Deployment simulation](https://openai.com/index/deployment-simulation/)
...
8. **RolloutBudget 重启连续性**：weighted counter 从零初始化，当前 commit 未见 hydrate。
...
Sol Ultra 做好长程任务的核心由七层共同构成：模型更能理解并持续执行目标；`max` 分配更多推理时计算；Ultra 扩大并行搜索宽度；persisted reasoning、WebSocket、Code Mode/PTC 提高多轮工具效率；Codex 用 rollout、compaction、WorldState、Goal、Agent graph 和 exec runtime 提供恢复骨架；仓库 Artifact 保存跨线程事实；验证与权限闭环约束完成与越界。
...
- 子 Agent规划提升主要表现为模型分解与复合编排的联合收益，公开分数只证明 bundle 点估计。
- 任务状态落盘来自多种载体；PlanUpdate 自身是 transient，Goal/rollout/WorldState/Artifact 承担真实恢复。
- token 限额没有统一路径；Goal budget 的 SQLite 跃迁最像预算状态落盘，compaction 与 TokenBudget处理上下文，RolloutBudget 和 usage limit处理其他停止语义。

长程 Agent 的工程本质是：**把一次模型调用变成可恢复、可验证、可审计、可停止的状态机。** Sol 提高了这个状态机每一步的语义质量；Codex Harness 决定这些能力能否跨数小时、多 Agent、窗口切换和故障持续兑现。
...
本报告保存为独立 final-v1。所有旧草稿和审计文件保持不变。官方网页结论绑定 2026-07-12 访问日，源码结论绑定 commit `26f5998e`。视觉结论以 [视觉证据审计 v1](sources/visual-evidence-audit.md) 和 [视觉复核 v2](sources/visual-evidence-audit-v2.md) 为准；任何未完成视觉读取的值继续标为未知。
